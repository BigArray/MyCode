...
#define ZSqlite Z_ModelSqliteKit

/// 数据库协议信息
@protocol Z_SqliteInfo <NSObject>

/// 自定义模型类数据库版本号
/** 注意：
 ***该返回值在改变数据模型属性类型/增加/删除属性时需要更改否则无法自动更新原来模型数据表字段以及类型***
 */
+ (NSString *)z_SqliteVersion;
 ...
 ***该加密功能需要引用SQLCipher三方库才支持***
 /// 引入方式有:
 *** 手动引入 ***
 *** pod 'Z_ModelSqliteKit/SQLCipher' ***
 */
+ (NSString *)z_SqlitePasswordKey;

/// 自定义数据表主键名称
/**
 *** 返回自定义主键名称默认主键:_id ***
 */
+ (NSString *)z_SqliteMainkey;
...
/**
 ...
 @return 返回忽略属性集合
 */
+ (NSArray *)z_IgnorePropertys;

@end

@interface Z_ModelSqliteKit : NSObject
...
/// example: [ZSqlite query:[Person class] order:@"by age desc/asc"];
...
/// example: [ZSqlite query:[Person class] limit:@"8"];
/// 对person数据表查询并且并且限制查询数量为8
/// example: [ZSqlite query:[Person class] limit:@"8 offset 8"];
...
/// example: [ZSqlite query:[Person class] where:@"age < 30" order:@"by age desc/asc"];
...
/// example: [ZSqlite query:[Person class] where:@"age <= 30" limit:@"8"];
/// 对person数据表查询age小于30岁并且限制查询数量为8
/// example: [ZSqlite query:[Person class] where:@"age <= 30" limit:@"8 offset 8"];
...
/// example: [ZSqlite query:[Person class] order:@"by age desc/asc" limit:@"8"];
/// 对person数据表查询并且根据age自动降序或者升序排序并且限制查询的数量为8
/// example: [ZSqlite query:[Person class] order:@"by age desc/asc" limit:@"8 offset 8"];
...
/// example: [ZSqlite query:[Person class] where:@"age <= 30" order:@"by age desc/asc" limit:@"8"];
/// 对person数据表查询age小于30岁并且根据age自动降序或者升序排序并且限制查询的数量为8
/// example: [ZSqlite query:[Person class] where:@"age <= 30" order:@"by age desc/asc" limit:@"8 offset 8"];
...
/**
 ...
 /// example: [ZSqlite query:Model.self sql:@"select cc.* from ( select tt.*,(select count(*)+1 from Chapter where chapter_id =tt.chapter_id and updateTime<tt.updateTime ) as group_id from Chapter tt) cc where cc.group_id<=7 order by updateTime desc"];
 */
 ...
 * /// example: [ZSqlite query:[Person class] sqliteFunc:@"max(age)"];  /// 获取Person表的最大age值
 * /// example: [ZSqlite query:[Person class] sqliteFunc:@"count(*)"];  /// 获取Person表的总记录条数
 */
 ...
 * @return 返回查询结果(如果结果条数 > 1返回Array , = 1返回单个值 , = 0返回nil)
 * /// example: [ZSqlite query:[Person class] sqliteFunc:@"max(age)" condition:@"where name = '北京'"];  /// 获取Person表name=北京集合中的的最大age值
 * /// example: [ZSqlite query:[Person class] sqliteFunc:@"count(*)" condition:@"where name = '北京'"];  /// 获取Person表name=北京集合中的总记录条数
 */
...
/**
 ...
 @param where 更新条件
 @return 是否成功
 /// 更新Person表在age字段大于25岁是的name值为Z，age为100岁
 /// example: [ZSqlite update:Person.self value:@"name = 'z', age = 100" where:@"age > 25"];
 */
