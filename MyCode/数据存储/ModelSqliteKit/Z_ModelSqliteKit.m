...
static const NSString * Z_String     = @"TEXT";
static const NSString * Z_Int        = @"INTERGER";
static const NSString * Z_Boolean    = @"INTERGER";
static const NSString * Z_Double     = @"DOUBLE";
static const NSString * Z_Float      = @"DOUBLE";
static const NSString * Z_Char       = @"NVARCHAR";
static const NSString * Z_Data       = @"BLOB";
static const NSString * Z_Array      = @"BLOB";
static const NSString * Z_Dictionary = @"BLOB";
static const NSString * Z_Date       = @"DOUBLE";
...
static sqlite3 * _z_database;

@interface Z_PropertyInfo : NSObject

@property (nonatomic, assign, readonly) Z_FieldType type;
@property (nonatomic, copy, readonly) NSString * name;
@property (nonatomic, assign, readonly) SEL setter;
@property (nonatomic, assign, readonly) SEL getter;
@end

@implementation Z_PropertyInfo

- (Z_PropertyInfo *)initWithType:(Z_FieldType)type
...
+ (Z_ModelSqliteKit *)shareInstance {
    static Z_ModelSqliteKit * instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [Z_ModelSqliteKit new];
    });
    return instance;
}

+ (NSString *)databaseCacheDirectory {
    return [NSString stringWithFormat:@"%@/Library/Caches/ZSqlite/",NSHomeDirectory()];
}

+ (Z_FieldType)parserFieldTypeWithAttr:(NSString *)attr {
    ...
    Z_FieldType field_type = _String;
...
+ (const NSString *)databaseFieldTypeWithType:(Z_FieldType)type {
    switch (type) {
        case _String:
            return Z_String;
        case _Int:
...
+ (NSDictionary *)parserSubModelObjectFieldsWithModelClass:(Class)model_class propertyName:(NSString *)main_property_name complete:(void(^)(NSString * key, Z_PropertyInfo * property_object))complete {
    ...
    SEL selector = @selector(z_IgnorePropertys);
    ...
    for (int i = 0; i < property_count; i++) {
        ...
        if ((ignore_propertys && [ignore_propertys containsObject:property_name_string]) || [property_name_string isEqualToString:@"zId"]) {
            continue;
        }
        ...
        Z_PropertyInfo * property_info = nil;
        if (property_attributes_list.count == 1) {
            // base type
            Z_FieldType type = [self parserFieldTypeWithAttr:property_attributes_list[0]];
            property_info = [[Z_PropertyInfo alloc] initWithType:type propertyName:property_name_string name:name];
        }else {
            // refernece type
            Class class_type = NSClassFromString(property_attributes_list[1]);
            if (class_type == [NSNumber class]) {
                property_info = [[Z_PropertyInfo alloc] initWithType:_Number propertyName:property_name_string name:name];
            }else if (class_type == [NSString class]) {
                property_info = [[Z_PropertyInfo alloc] initWithType:_String propertyName:property_name_string name:name];
            }else if (class_type == [NSData class]) {
                property_info = [[Z_PropertyInfo alloc] initWithType:_Data propertyName:property_name_string name:name];
            }else if (class_type == [NSArray class]) {
                property_info = [[Z_PropertyInfo alloc] initWithType:_Array propertyName:property_name_string name:name];
            }else if (class_type == [NSDictionary class]) {
                property_info = [[Z_PropertyInfo alloc] initWithType:_Dictionary propertyName:property_name_string name:name];
            }else if (class_type == [NSDate class]) {
                property_info = [[Z_PropertyInfo alloc] initWithType:_Date propertyName:property_name_string name:name];
                ...
                if (need_dictionary_save) {
                    [self parserSubModelObjectFieldsWithModelClass:class_type propertyName:name complete:^(NSString * key, Z_PropertyInfo *property_object) {
...
+ (sqlite_int64)getModelMaxIdWithClass:(Class)model_class {
    ...
    if (_z_database) {
        ...
        if (sqlite3_prepare_v2(_z_database, [select_sql UTF8String], -1, &pp_stmt, nil) == SQLITE_OK) {
...
+ (NSArray *)getModelFieldNameWithClass:(Class)model_class {
    ...
    if (_z_database) {
        ...
        if(sqlite3_prepare_v2(_z_database, [sql UTF8String], -1, &pp_stmt, NULL) == SQLITE_OK){
            while(sqlite3_step(pp_stmt) == SQLITE_ROW) {
...
+ (void)updateTableFieldWithModel:(Class)model_class
    ...
    @autoreleasepool {
        ...
        if (sqlite3_open([database_cache_path UTF8String], &_z_database) == SQLITE_OK) {
            ...
            [new_model_info enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, Z_PropertyInfo * obj, BOOL * _Nonnull stop) {
...
+ (BOOL)setKeyWithData:(NSData *)keyData {
    ...
    int rc = sqlite3_key(_z_database, [keyData bytes], (int)[keyData length]);
...
+ (BOOL)rekeyWithData:(NSData *)keyData {
    ...
    int rc = sqlite3_rekey(_z_database, [keyData bytes], (int)[keyData length]);
...
+ (NSString *)getMainKeyWithClass:(Class)model_class {
    NSString * main_key = [self exceSelector:@selector(z_SqliteMainkey) modelClass:model_class];
...
+ (void)decryptionSqlite:(Class)model_class {
#ifdef SQLITE_HAS_CODEC
    NSString * psw_key = [self exceSelector:@selector(z_SqlitePasswordKey) modelClass:model_class];
    if (psw_key && psw_key.length > 0) {
        ...
        if (![self setKey:is_update_psw ? old_psw : psw_key]) {
            [self log:@"给数据库加密失败, 请引入SQLCipher库并配置SQLITE_HAS_CODEC或者pod 'Z_ModelSqliteKit/SQLCipher'"];
        }else {
...
+ (BOOL)openTable:(Class)model_class {
    ...
    SEL VERSION = @selector(z_SqliteVersion);
...
+ (BOOL)createTable:(Class)model_class {
    ...
    if (field_dictionary.count > 0) {
        ...
        [field_dictionary enumerateKeysAndObjectsUsingBlock:^(NSString * field, Z_PropertyInfo * property_info, BOOL * _Nonnull stop) {
...
+ (BOOL)execSql:(NSString *)sql {
    BOOL result = sqlite3_exec(_z_database, [sql UTF8String], nil, nil, nil) == SQLITE_OK;
    if (!result) {
...
+ (BOOL)commonInsert:(id)model_object {
    ...
    [field_array enumerateObjectsUsingBlock:^(NSString * field, NSUInteger idx, BOOL * _Nonnull stop) {
        Z_PropertyInfo * property_info = field_dictionary[field];
    ...
    if (sqlite3_prepare_v2(_z_database, [insert_sql UTF8String], -1, &pp_stmt, nil) == SQLITE_OK) {
...
+ (NSArray *)commonQuery:(Class)model_class conditions:(NSArray *)conditions queryType:(Z_QueryType)query_type {
    NSString * table_name = NSStringFromClass(model_class);
...
+ (NSArray *)startSqlQuery:(Class)model_class sql:(NSString *)sql {
    ...
    sqlite3_stmt * pp_stmt = nil;
    if (sqlite3_prepare_v2(_z_database, [sql UTF8String], -1, &pp_stmt, nil) == SQLITE_OK) {
        ...
        while (sqlite3_step(pp_stmt) == SQLITE_ROW) {
            ...
            SEL z_id_sel = NSSelectorFromString(@"setzId:");
            if ([model_object respondsToSelector:z_id_sel]) {
                sqlite3_int64 value = sqlite3_column_int64(pp_stmt, 0);
                ((void (*)(id, SEL, int64_t))(void *) objc_msgSend)((id)model_object, z_id_sel, value);
            }
            for (int column = 1; column < colum_count; column++) {
                ...
                Z_PropertyInfo * property_info = field_dictionary[field_name];
...
+ (NSArray *)startQuery:(Class)model_class conditions:(NSArray *)conditions queryType:(Z_QueryType)query_type {
...
+ (NSArray *)queryModel:(Class)model_class conditions:(NSArray *)conditions queryType:(Z_QueryType)query_type {
...
+ (BOOL)updateModel:(id)model_object where:(NSString *)where {
    ...
    if (sqlite3_prepare_v2(_z_database, [update_sql UTF8String], -1, &pp_stmt, nil) == SQLITE_OK) {
        [field_array enumerateObjectsUsingBlock:^(id  _Nonnull field, NSUInteger idx, BOOL * _Nonnull stop) {
            Z_PropertyInfo * property_info = field_dictionary[field];
            id current_model_object = model_object;
